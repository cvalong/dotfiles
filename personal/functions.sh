...
# get local branches, remove spaces and sort them
# get merged prs
# find common branches
# delete the branches
delete_merged_branches() {
  echo "Deleting merged branches..."
  comm -12 <(git branch | grep -v '^\*\|^  main$\|^  master$' | tr -d ' ' | sort) <(gh pr list --state merged --limit 100 --json headRefName --jq '.[].headRefName' | sort) |
